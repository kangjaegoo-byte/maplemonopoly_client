...
void RoomPageList::AsyncData(const std::vector<RoomDTO>& _data)
{
	EnterCriticalSection(&m_roomListLock);
	m_data.clear();
	for (auto& item : _data)
...
void RoomPageList::EnterRoomEvent(int _index)
{
	EnterCriticalSection(&m_roomListLock);
	...
	RoomDTO enterRoom = m_data[_index];
	...
	MaplemonopolyApp::GetInstance()->GetUserInfo()->SetRoomSq(enterRoom.GetSq());

	// 서버로 WROOM 입장 요청 보내기 
	Network::GetInstance()->SendPacket((char*)&enterRoom, CLIENT_ETNER_WATING_ROOM_REQUEST, (PACKET_HEADER_SIZE + sizeof(RoomDTO)), 0);
	LeaveCriticalSection(&m_roomListLock);
	SceneManager::GetInstance()->MoveViewIndex(2);
}
